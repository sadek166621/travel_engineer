...
{{--                @endif--}}
                <div class="card-body card-block">
                    <form action="@isset($item){{ route('admin.place.update', $item->id) }}@else{{ route('admin.place.store') }}@endisset"
                            ...
                            <div class="col-12 col-md-9">
                                <select name="country_id" id="" class="form-control @error('country_id') is-invalid @enderror">
                                    ...
                                    @foreach($countries as $country)
                                        <option value="{{$country->id}}" @isset($item){{$item->country_id == $country->id ? 'selected':''}}@endisset>{{$country->name}}</option>
                                    @endforeach
                                </select>
                                @error('country_id')
